...
import { point, cleanPoints, toSegments, segmentsProperlyCross, getExpandedChildEdges, getExpandedChildShapes } from '../geometry/index.js';
import { visibilityRoute, pathIsClear } from '../process/routing/SequenceFlowRouting.js';
...
function enforceBoundaryVerticalExit(
  ...
  for (const candidateDock of targetDocks) {
    ...
    const route = visibilityRoute(
      ...
      sourceElement,
      element.targetRef,
      routedConnections.filter(({ flow }) => flow !== element)
    ) || visibilityRoute(
      stub,
      targetStub,
      obstacles,
...
function endpointRouteIsClear(
  ...
  return !toSegments(points).some(([ a, b ]) => {
    return routedConnections.some(connection => {
      if (connection.flow === ignoredFlow) {
        return false;
      }
