...
import { ROUTING_MARGIN, MAX_ROUTE_SEARCH_ATTEMPTS, MAX_LOCAL_U_CHANNEL_ATTEMPTS, ROUTE_OBSTACLE_INSET, LOCAL_U_OBSTACLE_CLEARANCE, VISIBILITY_GRAPH_TURN_PENALTY } from './Constants.js';
import {
  point,
  cleanPoints,
  toSegments,
...
function createVisibilitySegmentChecker(shapes, sourceElement, targetElement, routedConnections) {
  const obstacles = shapes
    .filter(({ element }) => element !== sourceElement && element !== targetElement)
    .map(({ rect }) => inset(rect, ROUTE_OBSTACLE_INSET));
