...
export function createSemanticPolicy(scope, records, graphEdges, boundaryEdges, allRecords) {
  ...
  alignLinkEventBands(records, bands);
...
function alignLinkEventBands(records, bands) {
  const links = new Map();
  ...
  for (const { throwEvent, catchEvent } of links.values()) {
    if (throwEvent && catchEvent) {
      bands.set(catchEvent, bands.get(throwEvent));
    }
  }
}
...
export function assignRanks(records, graphEdges, boundaryEdges, policy) {
  ...
  reserveDetachedBranchSpans(rank, outgoing, policy.spine, backEdges);

  // Boundary handlers enter the normal graph after their host rank is known.
  // Resolve their rank in the same fixed-point pass as normal flow so a host
  // whose rank is discovered late cannot leave its handler in an earlier rank.
  for (let iteration = 0; iteration < records.length; iteration++) {
    let changed = false;
...
function reserveDetachedBranchSpans(rank, outgoing, spine, backEdges) {
  ...
  for (const spineEdge of spine) {
    ...
    for (const branch of branches) {
      const pending = [ branch.targetRef ];
      const visited = new Set();
      let detached = true;
      let cyclic = false;
      let branchEnd = rank.get(branch.targetRef);
