...
export function orderParticipantsByMessageFlow(collaboration, participantShapes, endpointShapes) {
  ...
  do {
    ...
    for (const participant of participants) {
      ...
      let bestScore = messageFlowOrderScore(
        ordered,
        collaboration,
        participantShapes,
        endpointShapes
...
function messageFlowOrderScore(order, collaboration, participantShapes, endpointShapes) {
  ...
  const prioritizeAdjacency = (collaboration.messageFlows || []).some(messageFlow => {
    const source = findEndpointParticipant(
      resolveMessageFlowEndpoint(messageFlow.sourceRef, endpointShapes),
      collaboration
    );
    const target = findEndpointParticipant(
      resolveMessageFlowEndpoint(messageFlow.targetRef, endpointShapes),
      collaboration
    );
