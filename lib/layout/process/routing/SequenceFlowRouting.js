...
import { ROUTING_MARGIN, MAX_ROUTE_SEARCH_ATTEMPTS, MAX_LOCAL_U_CHANNEL_ATTEMPTS, ROUTE_OBSTACLE_INSET, LOCAL_U_OBSTACLE_CLEARANCE, VISIBILITY_GRAPH_TURN_PENALTY, MAX_VISIBILITY_GRAPH_POINTS } from '../../Constants.js';
import {
  point,
  cleanPoints,
  toSegments,
  ...
  if (crossBand) {
    ...
    if (centeredEndpoints) {
      ...
      for (let attempt = 1; attempt <= MAX_ROUTE_SEARCH_ATTEMPTS; attempt++) {
        const channelY = facingEnd.y + (targetAbove ? 1 : -1) * attempt * ROUTING_MARGIN;
...
export function visibilityRoute(
  ...
  const queue = [];
  const segmentIsClear = createVisibilitySegmentChecker(
    shapes,
    sourceElement,
    targetElement,
    routedConnections,
...
function createVisibilitySegmentChecker(
    shapes,
    sourceElement,
    targetElement,
    routedConnections,
  ...
  return (a, b) => {
    ...
    return !connections.some(connection => {
      return connection.segments.some(([ c, d ]) => {
        return (!allowPerpendicularCrossings && segmentsProperlyCross(a, b, c, d)) ||
          (!connection.sharedEndpoint && collinearOverlap(a, b, c, d));
      });
    });
  };
}
