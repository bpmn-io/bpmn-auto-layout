...
function ensureExitBottom(source, waypoints, [ row, col ]) {
  ...
  if (waypoints.length === 2) {
    const newStart = [
      ...
      { x: !baseSourceGrid ? (col + 1) * DEFAULT_CELL_WIDTH : (col + baseSourceGrid.getGridDimensions()[1] + 1) * DEFAULT_CELL_WIDTH, y: !baseSourceGrid ? (row + 0.5) * DEFAULT_CELL_HEIGHT : (row + baseSourceGrid.getGridDimensions()[0] ) * DEFAULT_CELL_HEIGHT },
    ];
