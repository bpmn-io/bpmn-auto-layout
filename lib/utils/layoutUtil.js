...
/**
     ...
     * @returns waypoints
     */
//connectElements(att, target, layoutGrid, shift)
export function connectElements(source, target, layoutGrid, shift) {
...
export function getBounds(element, row, col, shift, attachedTo) {
  ...
  //   width += colCount * DEFAULT_CELL_WIDTH;
  //   height += rowCount * DEFAULT_CELL_HEIGHT;
  // }

  const { x, y } = shift;

  // let { width: defaultWidth, height: defaultHeight } = getDefaultSize(element);
