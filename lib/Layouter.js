...
export class Layouter {
  ...
  generateDi(root, layoutGrid) {
    ...
    layoutGrid.elementsByPosition().forEach(({ element, row, col }) => {
      const DIs = this.handle('createDi', { element, row, col, layoutGrid, diFactory });

      planeElement.push(...(DIs.flat()));
    });

    // Step 2: Create DI for all connections
    layoutGrid.elementsByPosition().forEach(({ element, row, col }) => {
      const DIs = this.handle('createConnectionDi', { element, row, col, layoutGrid, diFactory });
      planeElement.push(...(DIs.flat()));
    });
  }
