...
import { VERTICAL_GAP, SUB_PROCESS_PADDING, PARTICIPANT_HEADER_WIDTH, MIN_PARTICIPANT_WIDTH, MIN_PARTICIPANT_HEIGHT, MIN_SUB_PROCESS_WIDTH, MIN_SUB_PROCESS_HEIGHT } from './layout/Constants.js';
import {
  isArtifact,
  isExteriorArtifact,
...
export class Layouter {
  ...
  async layoutProcess(xml) {
    ...
    if (!root) {
      return (await this.moddle.toXML(definitions, { format: true })).xml;
    }
  ...
  layoutScope(scope) {
    ...
    validateLaneFlowPaths(scope, sequenceFlows);
  ...
  placeArtifacts(records, associations, layout) {
    ...
    const graphElements = new Set(layout.shapes.keys());
    ...
    for (const record of artifactRecords) {
      ...
      const ownerBounds = references.length
        ? layout.shapes.get(references[0].owner)
        : null;
      const container = ownerBounds
        ? findContainingLaneBounds(ownerBounds, layout.shapes)
        : null;
      const processContainer = !container && isExteriorArtifact(record.element)
        ? bounds(
      ...
      placedArtifactBounds.push(placement);
    }
