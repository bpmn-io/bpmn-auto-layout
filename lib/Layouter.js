...
import { HORIZONTAL_GAP, VERTICAL_GAP, SUB_PROCESS_PADDING, PARTICIPANT_HEADER_WIDTH, MIN_PARTICIPANT_WIDTH, MIN_PARTICIPANT_HEIGHT, MIN_SUB_PROCESS_WIDTH, MIN_SUB_PROCESS_HEIGHT, EXPANDED_SUBPROCESS_ANNOTATION_CLEARANCE, EXPANDED_SUBPROCESS_LABEL_HEIGHT, MAX_ARTIFACT_SEARCH_OFFSET } from './layout/Constants.js';
import {
  isArtifact,
  isExteriorArtifact,
...
export class Layouter {
  ...
  layoutCollaboration(collaboration) {
    ...
    const messageFlowEndpoints = new Set((collaboration.messageFlows || [])
      .flatMap(messageFlow => [ messageFlow.sourceRef, messageFlow.targetRef ]));
    ...
    for (const participant of collaboration.participants || []) {
      ...
      const processLayout = this.layoutScope(process, true, messageFlowEndpoints);
  ...
  layoutScope(scope, participantProcess = false, messageFlowEndpoints = new Set()) {
    ...
    for (const record of records) {
      ...
      record.child = this.layoutScope(record.element, false, messageFlowEndpoints);
    ...
    if (participantProcess) {
      ...
      this.placeArtifacts(records, associations, layout, [], messageFlowEndpoints);
    }
    this.warnings.push(...layoutGroups(groups, layout));
  ...
  placeArtifacts(
      ...
      reservedVerticalEndpoints = new Set()) {
    ...
    const annotatedMessageEndpoints = new Set();
    ...
    for (const association of associations) {
      ...
      if (annotation && reservedVerticalEndpoints.has(owner)) {
        annotatedMessageEndpoints.add(owner);
      }
    }
    ...
    const reservedRoutes = [ ...annotatedMessageEndpoints ].flatMap(element => {
