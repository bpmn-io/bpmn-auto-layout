...
const PADDING_NODE = 'padding_node'
...
AutoLayout.prototype._explodeSubprocess = function (flowElement, flowElements) {
  ...
  */
  ...
  var self = this
  const subprocessNode = self._addTreeNode(flowElement, flowElements)
  self._buildTreeBreadFirstSearch(flowElement)
  if (flowElement.flowElements) {
    const subprocessStartEvents = getStartEvents(flowElement.flowElements)
    const subprocessStartNodes = subprocessStartEvents.map(startEvent => {
      const startNode = self.tree.getNodeByName(startEvent.id)
      return startNode
    })
    const subprocessEndEvents = getEndEvents(flowElement.flowElements)
    const subprocessEndNode = subprocessEndEvents.map(endEvent => {
    ...
    const outgoing = getOutgoingConnection(flowElement, flowElements)
    if (outgoing.length) {
    ...
    const childNodes = [...subprocessNode.children].filter(child => {
    ...
    subprocessStartNodes.forEach(subprocessStartNode => {
      const parents = [...subprocessStartNode.parents]
...
AutoLayout.prototype._addTreeNode = function (flowElement, flowElements) {
  ...
  if (!flowElement.build_marked) {
    ...
    if (incoming.length) {
      incoming.forEach(function (connection) {
        ...
        if (source.$type !== 'bpmn:SubProcess') {
          ...
          const subprocessEndEvents = getEndEvents(source.flowElements)
          const subprocessEnd = subprocessEndEvents[0] // assume only one end elements
    ...
    const size = getDefaultSize(flowElement)
...
AutoLayout.prototype._layoutElements = function (group, parentDi) {
  ...
  var getDefaultSize = this.DiFactory._getDefaultSize.bind(this.DiFactory)
  const _layoutTreeBreadFirstSearch = this._layoutTreeBreadFirstSearch.bind(this)
  var elements = group.elements
  var childrenDi = parentDi.get('planeElement')
  var elementDi
  var pos = {}
  var size
  const offsets = (group && group.offsets) || { x: 0, y: 0 }
  elements.forEach(function (element) {
    ...
    if (element.flowElements && element.$type === 'bpmn:SubProcess') {
      ...
      const offsetX = (element.bounds.x + element.bounds.width / 2) - (bounds.x + bounds.width / 2)
      const offsetY = (element.bounds.y + element.bounds.height / 2) - (bounds.y + bounds.height / 2)
      element.offsets = {
        x: offsetX + offsets.x,
        y: offsetY
      }
...
AutoLayout.prototype._layoutConnections = function (group, parentDi) {
  ...
  connections.forEach(function (connection) {
    ...
    var directTarget = DiFactoryUtils.is(sourceRef.$type, 'bpmn:SubProcess')
    ...
    if (sourceNode && sourceNode.children && sourceNode.children.length > 0 && !directTarget) {
      ...
      const oldPoints = connectionDi.waypoint
      const newPoints = [oldPoints[0]]
      // get node for sourceRef
      const pathNodes = getPathToTarget(sourceNode, targetRef.id)
      pathNodes.pop()
      if (DiFactoryUtils.is(sourceRef.$type, 'bpmn:Gateway')) {
        const start = oldPoints[0]
        const first = {
          x: pathNodes[1].XPosition + (group.offsets ? group.offsets.x : 0),
          y: pathNodes[1].YPosition + (group.offsets ? group.offsets.y : 0)
        }
        DiFactoryUtils.getBendpoints(start, first, 'h:h').forEach(bendpoint => {
      ...
      if (points.length > 0) {
        ...
        DiFactoryUtils.getBendpoints(points[points.length - 1], lastPoint, directions).forEach(bendpoint => {
          newPoints.push(createDiWaypoint(bendpoint))
        })
      }
  ...
  if (sourceNode.children) {
    ...
    sourceNode.children.forEach(source => {
      if (source.dsc === PADDING_NODE || source.dsc === targetId) {
        const path = getPathToTarget(source, targetId)
        if (path) {
          pathToTarget = path
        }
      }
...
function getStartEvents (flowElements) {
  return flowElements ? flowElements.filter(e => {
    return DiFactoryUtils.is(e.$type, 'bpmn:StartEvent')
  }) : []
}

function getEndEvents (flowElements) {
  return flowElements ? flowElements.filter(e => {
    return DiFactoryUtils.is(e.$type, 'bpmn:EndEvent')
  }) : []
}

function getOutgoingConnection (source, flowElements) {
  return flowElements ? flowElements.filter(e => {
    return DiFactoryUtils.is(e.$type, 'bpmn:SequenceFlow') && e.get('sourceRef').id === source.id
  }) : []
}

function getIncomingConnection (target, flowElements) {
  return flowElements ? flowElements.filter(e => {
    return DiFactoryUtils.is(e.$type, 'bpmn:SequenceFlow') && e.get('targetRef').id === target.id
  }) : []
}
