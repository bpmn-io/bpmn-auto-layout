...
function getExpandedBounds (element) {
  ...
  if (element.flowElements) {
    ...
    element.flowElements.forEach(flowElement => {
      const flowBounds = flowElement.bounds
      if (flowBounds) {
    ...
    element.flowElements.forEach(flowElement => {
      const flowBounds = flowElement.bounds
      if (flowBounds) {
        const newWidth = (flowBounds.x - bounds.x) + flowBounds.width
        const newHeight = (flowBounds.y - bounds.y) + flowBounds.height
...
function getOrientation (rect, reference, padding) {
  ...
  const right = rectOrientation.left - padding >= referenceOrientation.right
  const bottom = rectOrientation.top - padding >= referenceOrientation.bottom
  const left = rectOrientation.right + padding <= referenceOrientation.left
...
function is (type, expected) {
  const baseType = expected.split(':')[1]
  const findType = type.indexOf(baseType) !== -1
  return findType
}
