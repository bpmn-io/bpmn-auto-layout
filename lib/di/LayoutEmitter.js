...
// Boundary event label placement policy, relative to the event's center and top.
const BOUNDARY_EVENT_LABEL_WIDTH = 90;
const BOUNDARY_EVENT_LABEL_HEIGHT = 30;
const BOUNDARY_EVENT_LABEL_GAP = 5;
const BOUNDARY_EVENT_LABEL_VERTICAL_OFFSET = 35;
...
export function emitLayout(factory, layout, planeElements) {
  for (const [ element, rect ] of layout.shapes) {
    ...
    if (is(element, 'bpmn:BoundaryEvent') && element.name) {
      const host = layout.shapes.get(element.attachedToRef);

      if (host && rect.y + rect.height / 2 <= host.y) {
        const centerX = rect.x + rect.width / 2;
        const outgoingRoutePoints = [ ...layout.edges ]
          .find(([ flow ]) => flow.sourceRef === element)?.[1];
        const divergingWaypoint = outgoingRoutePoints?.find(point => point.x !== centerX);
        const labelX = divergingWaypoint?.x > centerX
          ? centerX - BOUNDARY_EVENT_LABEL_WIDTH - BOUNDARY_EVENT_LABEL_GAP
          : divergingWaypoint?.x < centerX
            ? centerX + BOUNDARY_EVENT_LABEL_GAP
            : centerX - BOUNDARY_EVENT_LABEL_WIDTH / 2;

        attrs.label = factory.createDiLabel(integerBounds({
          x: labelX,
          y: rect.y - BOUNDARY_EVENT_LABEL_VERTICAL_OFFSET,
          width: BOUNDARY_EVENT_LABEL_WIDTH,
          height: BOUNDARY_EVENT_LABEL_HEIGHT
        }));
      }
    }
