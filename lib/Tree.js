...
const TreeNode = function (id, dsc, w, h) {
  this.id = id
  this.dsc = dsc
  this.w = w
  this.h = h
...
TreeNode.prototype._getChildrenCenter = function (tree) {
  const first = this._getFirstChild()
  const last = this._getLastChild()
...
Tree.prototype._setNeighbors = function (node, level) {
  const tempNeighbour = this.previousLevelNode[level]
...
Tree.prototype.addParentToNode = function (nodeId, parentId) {
  ...
  const found = node.parents.find(parent => parent.id === parentId)
  if (found === undefined) {
    node.parents.push(parent)
    // confirm that the child hasnt been added to the parents children list
    const found = parent.children.find(child => child.id === nodeId)
    if (found === undefined) {
